...
class BalanceTrackingService:
    ...
    async def balance_tracking_query(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute balance tracking query against ClickHouse
        
        Args:
            params: Dictionary containing query parameters including the SQL query
            
        Returns:
            Dict containing query results
        """
        try:
            result = self.client.query(params["query"])
            return {
    ...
    async def get_balance_tracking_schema(self, assets: List[str] = None) -> Dict[str, Any]:
        """
        ...
        """
        try:
            tables = get_balance_tracking_tables()
            table_info = get_table_info(self.client, self.client.database, tables)
            
            # Add column information to each table
            for table in table_info:
                table.columns = get_column_info(self.client, self.client.database, table.name)
            
            schema = format_tables_to_schema(table_info)
            return create_balance_tracking_schema(schema, assets)
        except Exception as e:
